...
JavaScript (ES2022+, без TypeScript и без сборки) + Node 22+ (24 LTS тоже подходит) для сессий-песочниц; Docker (compose-файл создаётся в сессии 1); в браузере — нативные ES-модули без бандлера; `json-server` как мок-API (только `db.json`, ноль кода); собственный ~15-строчный сервер на `node:http` вместо Express/serve; тесты — встроенный `node --test`.
